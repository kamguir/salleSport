...
/**
 * Base class that represents a row from the 'ref_titre_entraineur' table.
 *
 * 
 *
 * @package    propel.generator.lib.model.om
 */
abstract class BaseRefTitreEntraineur extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'RefTitreEntraineurPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        RefTitreEntraineurPeer
	 */
	...
	/**
	 * The value for the titre_entraineur_libelle field.
	 * @var        string
	 */
	protected $titre_entraineur_libelle;
	...
	/**
	 * Get the [titre_entraineur_libelle] column value.
	 * 
	 * @return     string
	 */
	public function getTitreEntraineurLibelle()
	{
		return $this->titre_entraineur_libelle;
	}
	 ...
	 * @return     RefTitreEntraineur The current object (for fluent API support)
	 */
	public function setTitreEntraineurId($v)
	{
		...
		if ($this->titre_entraineur_id !== $v) {
			...
			$this->modifiedColumns[] = RefTitreEntraineurPeer::TITRE_ENTRAINEUR_ID;
		}
	...
	/**
	 * Set the value of [titre_entraineur_libelle] column.
	 * 
	 * @param      string $v new value
	 * @return     RefTitreEntraineur The current object (for fluent API support)
	 */
	public function setTitreEntraineurLibelle($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->titre_entraineur_libelle !== $v) {
			$this->titre_entraineur_libelle = $v;
			$this->modifiedColumns[] = RefTitreEntraineurPeer::TITRE_ENTRAINEUR_LIBELLE;
		}

		return $this;
	} // setTitreEntraineurLibelle()
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->titre_entraineur_id = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			$this->titre_entraineur_libelle = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->resetModified();
			...
			return $startcol + 2; // 2 = RefTitreEntraineurPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating RefTitreEntraineur object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = RefTitreEntraineurPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = RefTitreEntraineurQuery::create()
				->filterByPrimaryKey($this->getPrimaryKey());
			$ret = $this->preDelete($con);
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRefTitreEntraineur:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRefTitreEntraineur:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRefTitreEntraineur:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRefTitreEntraineur:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				RefTitreEntraineurPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		$this->modifiedColumns[] = RefTitreEntraineurPeer::TITRE_ENTRAINEUR_ID;
		if (null !== $this->titre_entraineur_id) {
		...
		if ($this->isColumnModified(RefTitreEntraineurPeer::TITRE_ENTRAINEUR_ID)) {
			$modifiedColumns[':p' . $index++]  = '`TITRE_ENTRAINEUR_ID`';
		}
		if ($this->isColumnModified(RefTitreEntraineurPeer::TITRE_ENTRAINEUR_LIBELLE)) {
			$modifiedColumns[':p' . $index++]  = '`TITRE_ENTRAINEUR_LIBELLE`';
		}

		$sql = sprintf(
			'INSERT INTO `ref_titre_entraineur` (%s) VALUES (%s)',
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					case '`TITRE_ENTRAINEUR_ID`':
						...
						break;
					case '`TITRE_ENTRAINEUR_LIBELLE`':
						$stmt->bindValue($identifier, $this->titre_entraineur_libelle, PDO::PARAM_STR);
						break;
				}
			}
			$stmt->execute();
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = RefTitreEntraineurPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = RefTitreEntraineurPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			case 0:
				...
				break;
			case 1:
				return $this->getTitreEntraineurLibelle();
				break;
			default:
				return null;
				break;
	...
	{
		if (isset($alreadyDumpedObjects['RefTitreEntraineur'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['RefTitreEntraineur'][$this->getPrimaryKey()] = true;
		$keys = RefTitreEntraineurPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getTitreEntraineurId(),
			$keys[1] => $this->getTitreEntraineurLibelle(),
		);
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			case 0:
				...
				break;
			case 1:
				$this->setTitreEntraineurLibelle($value);
				break;
		} // switch()
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setTitreEntraineurId($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setTitreEntraineurLibelle($arr[$keys[1]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(RefTitreEntraineurPeer::TITRE_ENTRAINEUR_ID)) $criteria->add(RefTitreEntraineurPeer::TITRE_ENTRAINEUR_ID, $this->titre_entraineur_id);
		if ($this->isColumnModified(RefTitreEntraineurPeer::TITRE_ENTRAINEUR_LIBELLE)) $criteria->add(RefTitreEntraineurPeer::TITRE_ENTRAINEUR_LIBELLE, $this->titre_entraineur_libelle);
	...
	{
		$criteria = new Criteria(RefTitreEntraineurPeer::DATABASE_NAME);
		$criteria->add(RefTitreEntraineurPeer::TITRE_ENTRAINEUR_ID, $this->titre_entraineur_id);
	...
	/**
	 * Generic method to set the primary key (titre_entraineur_id column).
	 *
	...
	{
		return null === $this->getTitreEntraineurId();
	}
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of RefTitreEntraineur (or compatible) type.
	...
	{
		$copyObj->setTitreEntraineurLibelle($this->getTitreEntraineurLibelle());
		...
		if ($makeNew) {
			$copyObj->setNew(true);
			$copyObj->setTitreEntraineurId(NULL); // this is a auto-increment column, so set to default value
		}
	}
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     RefTitreEntraineur Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     RefTitreEntraineurPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new RefTitreEntraineurPeer();
		}
		return self::$peer;
	}
	 ...
	 * If this RefTitreEntraineur is new, it will return
		...
		if(null === $this->collTblCeintures || null !== $criteria) {
			if ($this->isNew() && null === $this->collTblCeintures) {
				...
				$collTblCeintures = TblCeintureQuery::create(null, $criteria)
					->filterByRefTitreEntraineur($this)
					->find($con);
				if (null !== $criteria) {
	...
	public function setTblCeintures(PropelCollection $tblCeintures, PropelPDO $con = null)
	{
		...
		foreach ($tblCeintures as $tblCeinture) {
			...
			if ($tblCeinture->isNew()) {
				$tblCeinture->setRefTitreEntraineur($this);
			}
	 ...
	 * Otherwise if this RefTitreEntraineur is new, it will return
	 * an empty collection; or if this RefTitreEntraineur has previously
	 * been saved, it will retrieve related TblCeintures from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in RefTitreEntraineur.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	...
	public function clear()
	{
		...
		$this->titre_entraineur_libelle = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
	...
	{
		return (string) $this->exportTo(RefTitreEntraineurPeer::DEFAULT_STRING_FORMAT);
	}
	...
	public function __call($name, $params)
	{
		...
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseRefTitreEntraineur:' . $name))
		{
