...
/**
 * Base class that represents a row from the 'ref_type_adherent' table.
 *
 * 
 *
 * @package    propel.generator.lib.model.om
 */
abstract class BaseRefTypeAdherent extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'RefTypeAdherentPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        RefTypeAdherentPeer
	 */
	...
	/**
	 * The value for the libelle_type_adherent field.
	 * @var        string
	 */
	protected $libelle_type_adherent;
	...
	/**
	 * Get the [libelle_type_adherent] column value.
	 * 
	 * @return     string
	 */
	public function getLibelleTypeAdherent()
	{
		return $this->libelle_type_adherent;
	}
	 ...
	 * @return     RefTypeAdherent The current object (for fluent API support)
	 */
	public function setIdTypeAdherent($v)
	{
		...
		if ($this->id_type_adherent !== $v) {
			...
			$this->modifiedColumns[] = RefTypeAdherentPeer::ID_TYPE_ADHERENT;
		}
	...
	/**
	 * Set the value of [libelle_type_adherent] column.
	 * 
	 * @param      string $v new value
	 * @return     RefTypeAdherent The current object (for fluent API support)
	 */
	public function setLibelleTypeAdherent($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->libelle_type_adherent !== $v) {
			$this->libelle_type_adherent = $v;
			$this->modifiedColumns[] = RefTypeAdherentPeer::LIBELLE_TYPE_ADHERENT;
		}

		return $this;
	} // setLibelleTypeAdherent()
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->id_type_adherent = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			$this->libelle_type_adherent = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->resetModified();
			...
			return $startcol + 2; // 2 = RefTypeAdherentPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating RefTypeAdherent object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = RefTypeAdherentPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = RefTypeAdherentQuery::create()
				->filterByPrimaryKey($this->getPrimaryKey());
			$ret = $this->preDelete($con);
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRefTypeAdherent:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRefTypeAdherent:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRefTypeAdherent:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRefTypeAdherent:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				RefTypeAdherentPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(RefTypeAdherentPeer::ID_TYPE_ADHERENT)) {
			$modifiedColumns[':p' . $index++]  = '`ID_TYPE_ADHERENT`';
		}
		if ($this->isColumnModified(RefTypeAdherentPeer::LIBELLE_TYPE_ADHERENT)) {
			$modifiedColumns[':p' . $index++]  = '`LIBELLE_TYPE_ADHERENT`';
		}

		$sql = sprintf(
			'INSERT INTO `ref_type_adherent` (%s) VALUES (%s)',
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					case '`ID_TYPE_ADHERENT`':
						...
						break;
					case '`LIBELLE_TYPE_ADHERENT`':
						$stmt->bindValue($identifier, $this->libelle_type_adherent, PDO::PARAM_STR);
						break;
				}
			}
			$stmt->execute();
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = RefTypeAdherentPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = RefTypeAdherentPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['RefTypeAdherent'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['RefTypeAdherent'][$this->getPrimaryKey()] = true;
		$keys = RefTypeAdherentPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getIdTypeAdherent(),
			$keys[1] => $this->getLibelleTypeAdherent(),
		);
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			case 0:
				...
				break;
			case 1:
				$this->setLibelleTypeAdherent($value);
				break;
		} // switch()
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setIdTypeAdherent($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setLibelleTypeAdherent($arr[$keys[1]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(RefTypeAdherentPeer::ID_TYPE_ADHERENT)) $criteria->add(RefTypeAdherentPeer::ID_TYPE_ADHERENT, $this->id_type_adherent);
		if ($this->isColumnModified(RefTypeAdherentPeer::LIBELLE_TYPE_ADHERENT)) $criteria->add(RefTypeAdherentPeer::LIBELLE_TYPE_ADHERENT, $this->libelle_type_adherent);
	...
	{
		$criteria = new Criteria(RefTypeAdherentPeer::DATABASE_NAME);
		$criteria->add(RefTypeAdherentPeer::ID_TYPE_ADHERENT, $this->id_type_adherent);
	...
	/**
	 * Generic method to set the primary key (id_type_adherent column).
	 *
	...
	{
		return null === $this->getIdTypeAdherent();
	}
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of RefTypeAdherent (or compatible) type.
	...
	{
		$copyObj->setLibelleTypeAdherent($this->getLibelleTypeAdherent());
		...
		if ($makeNew) {
			$copyObj->setNew(true);
			$copyObj->setIdTypeAdherent(NULL); // this is a auto-increment column, so set to default value
		}
	}
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     RefTypeAdherent Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     RefTypeAdherentPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new RefTypeAdherentPeer();
		}
		return self::$peer;
	}
	 ...
	 * If this RefTypeAdherent is new, it will return
	...
	public function setTblAdherents(PropelCollection $tblAdherents, PropelPDO $con = null)
	{
		...
		foreach ($tblAdherents as $tblAdherent) {
			...
			if ($tblAdherent->isNew()) {
				$tblAdherent->setRefTypeAdherent($this);
			}
	 ...
	 * Otherwise if this RefTypeAdherent is new, it will return
	 * an empty collection; or if this RefTypeAdherent has previously
	 * been saved, it will retrieve related TblAdherents from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in RefTypeAdherent.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	...
	public function clear()
	{
		...
		$this->libelle_type_adherent = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
	...
	{
		return (string) $this->exportTo(RefTypeAdherentPeer::DEFAULT_STRING_FORMAT);
	}
	...
	public function __call($name, $params)
	{
		...
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseRefTypeAdherent:' . $name))
		{
