...
/**
 * Base class that represents a row from the 'ref_grade_ceinture' table.
 *
 * 
 *
 * @package    propel.generator.lib.model.om
 */
abstract class BaseRefGradeCeinture extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'RefGradeCeinturePeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        RefGradeCeinturePeer
	 */
	...
	/**
	 * The value for the grade_niveau field.
	 * @var        string
	 */
	protected $grade_niveau;
	...
	/**
	 * Get the [grade_niveau] column value.
	 * 
	 * @return     string
	 */
	public function getGradeNiveau()
	{
		return $this->grade_niveau;
	}
	 ...
	 * @return     RefGradeCeinture The current object (for fluent API support)
	 */
	public function setGradeId($v)
	{
		...
		if ($this->grade_id !== $v) {
			$this->grade_id = $v;
			$this->modifiedColumns[] = RefGradeCeinturePeer::GRADE_ID;
		}
	...
	/**
	 * Set the value of [grade_niveau] column.
	 * 
	 * @param      string $v new value
	 * @return     RefGradeCeinture The current object (for fluent API support)
	 */
	public function setGradeNiveau($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->grade_niveau !== $v) {
			$this->grade_niveau = $v;
			$this->modifiedColumns[] = RefGradeCeinturePeer::GRADE_NIVEAU;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->grade_id = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			$this->grade_niveau = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->resetModified();
			...
			return $startcol + 2; // 2 = RefGradeCeinturePeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating RefGradeCeinture object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = RefGradeCeinturePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = RefGradeCeintureQuery::create()
				->filterByPrimaryKey($this->getPrimaryKey());
			$ret = $this->preDelete($con);
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRefGradeCeinture:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRefGradeCeinture:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRefGradeCeinture:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRefGradeCeinture:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				RefGradeCeinturePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(RefGradeCeinturePeer::GRADE_ID)) {
			$modifiedColumns[':p' . $index++]  = '`GRADE_ID`';
		}
		if ($this->isColumnModified(RefGradeCeinturePeer::GRADE_NIVEAU)) {
			$modifiedColumns[':p' . $index++]  = '`GRADE_NIVEAU`';
		}

		$sql = sprintf(
			'INSERT INTO `ref_grade_ceinture` (%s) VALUES (%s)',
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					case '`GRADE_ID`':
						...
						break;
					case '`GRADE_NIVEAU`':
						$stmt->bindValue($identifier, $this->grade_niveau, PDO::PARAM_STR);
						break;
				}
			}
			$stmt->execute();
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = RefGradeCeinturePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = RefGradeCeinturePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['RefGradeCeinture'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['RefGradeCeinture'][$this->getPrimaryKey()] = true;
		$keys = RefGradeCeinturePeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getGradeId(),
			$keys[1] => $this->getGradeNiveau(),
		);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setGradeId($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setGradeNiveau($arr[$keys[1]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(RefGradeCeinturePeer::GRADE_ID)) $criteria->add(RefGradeCeinturePeer::GRADE_ID, $this->grade_id);
		if ($this->isColumnModified(RefGradeCeinturePeer::GRADE_NIVEAU)) $criteria->add(RefGradeCeinturePeer::GRADE_NIVEAU, $this->grade_niveau);
	...
	{
		$criteria = new Criteria(RefGradeCeinturePeer::DATABASE_NAME);
		$criteria->add(RefGradeCeinturePeer::GRADE_ID, $this->grade_id);
	...
	/**
	 * Generic method to set the primary key (grade_id column).
	 *
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of RefGradeCeinture (or compatible) type.
	...
	{
		$copyObj->setGradeNiveau($this->getGradeNiveau());
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     RefGradeCeinture Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     RefGradeCeinturePeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new RefGradeCeinturePeer();
		}
		return self::$peer;
	}
	 ...
	 * If this RefGradeCeinture is new, it will return
	...
	public function setTblCeintures(PropelCollection $tblCeintures, PropelPDO $con = null)
	{
		...
		foreach ($tblCeintures as $tblCeinture) {
			...
			if ($tblCeinture->isNew()) {
				$tblCeinture->setRefGradeCeinture($this);
			}
	 ...
	 * Otherwise if this RefGradeCeinture is new, it will return
	 * an empty collection; or if this RefGradeCeinture has previously
	 * been saved, it will retrieve related TblCeintures from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in RefGradeCeinture.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	...
	{
		return (string) $this->exportTo(RefGradeCeinturePeer::DEFAULT_STRING_FORMAT);
	}
	...
	public function __call($name, $params)
	{
		...
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseRefGradeCeinture:' . $name))
		{
