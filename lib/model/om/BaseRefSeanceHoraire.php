...
/**
 * Base class that represents a row from the 'ref_seance_horaire' table.
 *
 * 
 *
 * @package    propel.generator.lib.model.om
 */
abstract class BaseRefSeanceHoraire extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'RefSeanceHorairePeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        RefSeanceHorairePeer
	 */
	...
	/**
	 * The value for the seance_horaire field.
	 * @var        string
	 */
	protected $seance_horaire;
	...
	/**
	 * Get the [seance_horaire] column value.
	 * 
	 * @return     string
	 */
	public function getSeanceHoraire()
	{
		return $this->seance_horaire;
	}
	 ...
	 * @return     RefSeanceHoraire The current object (for fluent API support)
	 */
	public function setSeanceHoraireId($v)
	{
		...
		if ($this->seance_horaire_id !== $v) {
			...
			$this->modifiedColumns[] = RefSeanceHorairePeer::SEANCE_HORAIRE_ID;
		}
	...
	/**
	 * Set the value of [seance_horaire] column.
	 * 
	 * @param      string $v new value
	 * @return     RefSeanceHoraire The current object (for fluent API support)
	 */
	public function setSeanceHoraire($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->seance_horaire !== $v) {
			$this->seance_horaire = $v;
			$this->modifiedColumns[] = RefSeanceHorairePeer::SEANCE_HORAIRE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->seance_horaire_id = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			$this->seance_horaire = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->resetModified();
			...
			return $startcol + 2; // 2 = RefSeanceHorairePeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating RefSeanceHoraire object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = RefSeanceHorairePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = RefSeanceHoraireQuery::create()
				->filterByPrimaryKey($this->getPrimaryKey());
			$ret = $this->preDelete($con);
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRefSeanceHoraire:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRefSeanceHoraire:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRefSeanceHoraire:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRefSeanceHoraire:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				RefSeanceHorairePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		$this->modifiedColumns[] = RefSeanceHorairePeer::SEANCE_HORAIRE_ID;
		if (null !== $this->seance_horaire_id) {
		...
		if ($this->isColumnModified(RefSeanceHorairePeer::SEANCE_HORAIRE_ID)) {
			$modifiedColumns[':p' . $index++]  = '`SEANCE_HORAIRE_ID`';
		}
		if ($this->isColumnModified(RefSeanceHorairePeer::SEANCE_HORAIRE)) {
			$modifiedColumns[':p' . $index++]  = '`SEANCE_HORAIRE`';
		}

		$sql = sprintf(
			'INSERT INTO `ref_seance_horaire` (%s) VALUES (%s)',
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					case '`SEANCE_HORAIRE_ID`':
						...
						break;
					case '`SEANCE_HORAIRE`':
						$stmt->bindValue($identifier, $this->seance_horaire, PDO::PARAM_STR);
						break;
				}
			}
			$stmt->execute();
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = RefSeanceHorairePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = RefSeanceHorairePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['RefSeanceHoraire'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['RefSeanceHoraire'][$this->getPrimaryKey()] = true;
		$keys = RefSeanceHorairePeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getSeanceHoraireId(),
			$keys[1] => $this->getSeanceHoraire(),
		);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setSeanceHoraireId($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setSeanceHoraire($arr[$keys[1]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(RefSeanceHorairePeer::SEANCE_HORAIRE_ID)) $criteria->add(RefSeanceHorairePeer::SEANCE_HORAIRE_ID, $this->seance_horaire_id);
		if ($this->isColumnModified(RefSeanceHorairePeer::SEANCE_HORAIRE)) $criteria->add(RefSeanceHorairePeer::SEANCE_HORAIRE, $this->seance_horaire);
	...
	{
		$criteria = new Criteria(RefSeanceHorairePeer::DATABASE_NAME);
		$criteria->add(RefSeanceHorairePeer::SEANCE_HORAIRE_ID, $this->seance_horaire_id);
	...
	/**
	 * Generic method to set the primary key (seance_horaire_id column).
	 *
	...
	{
		return null === $this->getSeanceHoraireId();
	}
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of RefSeanceHoraire (or compatible) type.
	...
	{
		$copyObj->setSeanceHoraire($this->getSeanceHoraire());
		...
		if ($makeNew) {
			$copyObj->setNew(true);
			$copyObj->setSeanceHoraireId(NULL); // this is a auto-increment column, so set to default value
		}
	}
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     RefSeanceHoraire Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     RefSeanceHorairePeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new RefSeanceHorairePeer();
		}
		return self::$peer;
	}
	 ...
	 * If this RefSeanceHoraire is new, it will return
	...
	public function setTblAdherents(PropelCollection $tblAdherents, PropelPDO $con = null)
	{
		...
		foreach ($tblAdherents as $tblAdherent) {
			...
			if ($tblAdherent->isNew()) {
				$tblAdherent->setRefSeanceHoraire($this);
			}
	 ...
	 * Otherwise if this RefSeanceHoraire is new, it will return
	 * an empty collection; or if this RefSeanceHoraire has previously
	 * been saved, it will retrieve related TblAdherents from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in RefSeanceHoraire.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	...
	{
		return (string) $this->exportTo(RefSeanceHorairePeer::DEFAULT_STRING_FORMAT);
	}
	...
	public function __call($name, $params)
	{
		...
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseRefSeanceHoraire:' . $name))
		{
