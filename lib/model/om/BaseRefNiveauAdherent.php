...
/**
 * Base class that represents a row from the 'ref_niveau_adherent' table.
 *
 * 
 *
 * @package    propel.generator.lib.model.om
 */
abstract class BaseRefNiveauAdherent extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'RefNiveauAdherentPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        RefNiveauAdherentPeer
	 */
	...
	/**
	 * The value for the niveau_adherent_libelle field.
	 * @var        string
	 */
	protected $niveau_adherent_libelle;
	...
	/**
	 * Get the [niveau_adherent_libelle] column value.
	 * 
	 * @return     string
	 */
	public function getNiveauAdherentLibelle()
	{
		return $this->niveau_adherent_libelle;
	}
	 ...
	 * @return     RefNiveauAdherent The current object (for fluent API support)
	 */
	public function setNiveauAdherentId($v)
	{
		...
		if ($this->niveau_adherent_id !== $v) {
			...
			$this->modifiedColumns[] = RefNiveauAdherentPeer::NIVEAU_ADHERENT_ID;
		}
	...
	/**
	 * Set the value of [niveau_adherent_libelle] column.
	 * 
	 * @param      string $v new value
	 * @return     RefNiveauAdherent The current object (for fluent API support)
	 */
	public function setNiveauAdherentLibelle($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->niveau_adherent_libelle !== $v) {
			$this->niveau_adherent_libelle = $v;
			$this->modifiedColumns[] = RefNiveauAdherentPeer::NIVEAU_ADHERENT_LIBELLE;
		}

		return $this;
	} // setNiveauAdherentLibelle()
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->niveau_adherent_id = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			$this->niveau_adherent_libelle = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->resetModified();
			...
			return $startcol + 2; // 2 = RefNiveauAdherentPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating RefNiveauAdherent object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = RefNiveauAdherentPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = RefNiveauAdherentQuery::create()
				->filterByPrimaryKey($this->getPrimaryKey());
			$ret = $this->preDelete($con);
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRefNiveauAdherent:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRefNiveauAdherent:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRefNiveauAdherent:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRefNiveauAdherent:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				RefNiveauAdherentPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		$this->modifiedColumns[] = RefNiveauAdherentPeer::NIVEAU_ADHERENT_ID;
		if (null !== $this->niveau_adherent_id) {
		...
		if ($this->isColumnModified(RefNiveauAdherentPeer::NIVEAU_ADHERENT_ID)) {
			$modifiedColumns[':p' . $index++]  = '`NIVEAU_ADHERENT_ID`';
		}
		if ($this->isColumnModified(RefNiveauAdherentPeer::NIVEAU_ADHERENT_LIBELLE)) {
			$modifiedColumns[':p' . $index++]  = '`NIVEAU_ADHERENT_LIBELLE`';
		}

		$sql = sprintf(
			'INSERT INTO `ref_niveau_adherent` (%s) VALUES (%s)',
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					case '`NIVEAU_ADHERENT_ID`':
						...
						break;
					case '`NIVEAU_ADHERENT_LIBELLE`':
						$stmt->bindValue($identifier, $this->niveau_adherent_libelle, PDO::PARAM_STR);
						break;
				}
			}
			$stmt->execute();
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = RefNiveauAdherentPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = RefNiveauAdherentPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			case 0:
				...
				break;
			case 1:
				return $this->getNiveauAdherentLibelle();
				break;
			default:
				return null;
				break;
	...
	{
		if (isset($alreadyDumpedObjects['RefNiveauAdherent'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['RefNiveauAdherent'][$this->getPrimaryKey()] = true;
		$keys = RefNiveauAdherentPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getNiveauAdherentId(),
			$keys[1] => $this->getNiveauAdherentLibelle(),
		);
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			case 0:
				...
				break;
			case 1:
				$this->setNiveauAdherentLibelle($value);
				break;
		} // switch()
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setNiveauAdherentId($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setNiveauAdherentLibelle($arr[$keys[1]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(RefNiveauAdherentPeer::NIVEAU_ADHERENT_ID)) $criteria->add(RefNiveauAdherentPeer::NIVEAU_ADHERENT_ID, $this->niveau_adherent_id);
		if ($this->isColumnModified(RefNiveauAdherentPeer::NIVEAU_ADHERENT_LIBELLE)) $criteria->add(RefNiveauAdherentPeer::NIVEAU_ADHERENT_LIBELLE, $this->niveau_adherent_libelle);
	...
	{
		$criteria = new Criteria(RefNiveauAdherentPeer::DATABASE_NAME);
		$criteria->add(RefNiveauAdherentPeer::NIVEAU_ADHERENT_ID, $this->niveau_adherent_id);
	...
	/**
	 * Generic method to set the primary key (niveau_adherent_id column).
	 *
	...
	{
		return null === $this->getNiveauAdherentId();
	}
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of RefNiveauAdherent (or compatible) type.
	...
	{
		$copyObj->setNiveauAdherentLibelle($this->getNiveauAdherentLibelle());
		...
		if ($makeNew) {
			$copyObj->setNew(true);
			$copyObj->setNiveauAdherentId(NULL); // this is a auto-increment column, so set to default value
		}
	}
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     RefNiveauAdherent Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     RefNiveauAdherentPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new RefNiveauAdherentPeer();
		}
		return self::$peer;
	}
	 ...
	 * If this RefNiveauAdherent is new, it will return
		...
		if(null === $this->collTblAdherents || null !== $criteria) {
			if ($this->isNew() && null === $this->collTblAdherents) {
				...
				$collTblAdherents = TblAdherentQuery::create(null, $criteria)
					->filterByRefNiveauAdherent($this)
					->find($con);
				if (null !== $criteria) {
	...
	public function setTblAdherents(PropelCollection $tblAdherents, PropelPDO $con = null)
	{
		...
		foreach ($tblAdherents as $tblAdherent) {
			...
			if ($tblAdherent->isNew()) {
				$tblAdherent->setRefNiveauAdherent($this);
			}
	 ...
	 * Otherwise if this RefNiveauAdherent is new, it will return
	 * an empty collection; or if this RefNiveauAdherent has previously
	 * been saved, it will retrieve related TblAdherents from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in RefNiveauAdherent.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	...
	public function clear()
	{
		...
		$this->niveau_adherent_libelle = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
	...
	{
		return (string) $this->exportTo(RefNiveauAdherentPeer::DEFAULT_STRING_FORMAT);
	}
	...
	public function __call($name, $params)
	{
		...
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseRefNiveauAdherent:' . $name))
		{
