...
/**
 * Base class that represents a row from the 'ref_jour' table.
 *
 * 
 *
 * @package    propel.generator.lib.model.om
 */
abstract class BaseRefJour extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'RefJourPeer';
	...
	/**
	 * The value for the libelle_jour field.
	 * @var        string
	 */
	protected $libelle_jour;
	...
	/**
	 * Get the [libelle_jour] column value.
	 * 
	 * @return     string
	 */
	 ...
	 * @return     RefJour The current object (for fluent API support)
	 */
	public function setIdJour($v)
	{
		...
		if ($this->id_jour !== $v) {
			$this->id_jour = $v;
			$this->modifiedColumns[] = RefJourPeer::ID_JOUR;
		}
	...
	/**
	 * Set the value of [libelle_jour] column.
	 * 
	 * @param      string $v new value
	 * @return     RefJour The current object (for fluent API support)
	 */
	public function setLibelleJour($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->libelle_jour !== $v) {
			$this->libelle_jour = $v;
			$this->modifiedColumns[] = RefJourPeer::LIBELLE_JOUR;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->libelle_jour = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->resetModified();
			...
			return $startcol + 2; // 2 = RefJourPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating RefJour object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = RefJourPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = RefJourQuery::create()
				->filterByPrimaryKey($this->getPrimaryKey());
			$ret = $this->preDelete($con);
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRefJour:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRefJour:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRefJour:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRefJour:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				RefJourPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(RefJourPeer::ID_JOUR)) {
			$modifiedColumns[':p' . $index++]  = '`ID_JOUR`';
		}
		if ($this->isColumnModified(RefJourPeer::LIBELLE_JOUR)) {
			$modifiedColumns[':p' . $index++]  = '`LIBELLE_JOUR`';
		}

		$sql = sprintf(
			'INSERT INTO `ref_jour` (%s) VALUES (%s)',
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					case '`ID_JOUR`':
						...
						break;
					case '`LIBELLE_JOUR`':
						$stmt->bindValue($identifier, $this->libelle_jour, PDO::PARAM_STR);
						break;
				}
			}
			$stmt->execute();
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = RefJourPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = RefJourPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['RefJour'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['RefJour'][$this->getPrimaryKey()] = true;
		$keys = RefJourPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getIdJour(),
			$keys[1] => $this->getLibelleJour(),
		);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setLibelleJour($arr[$keys[1]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(RefJourPeer::ID_JOUR)) $criteria->add(RefJourPeer::ID_JOUR, $this->id_jour);
		if ($this->isColumnModified(RefJourPeer::LIBELLE_JOUR)) $criteria->add(RefJourPeer::LIBELLE_JOUR, $this->libelle_jour);
	...
	{
		$criteria = new Criteria(RefJourPeer::DATABASE_NAME);
		$criteria->add(RefJourPeer::ID_JOUR, $this->id_jour);
	...
	/**
	 * Generic method to set the primary key (id_jour column).
	 *
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of RefJour (or compatible) type.
	...
	{
		$copyObj->setLibelleJour($this->getLibelleJour());
		...
		if ($makeNew) {
			$copyObj->setNew(true);
			$copyObj->setIdJour(NULL); // this is a auto-increment column, so set to default value
		}
	}
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     RefJour Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * If this RefJour is new, it will return
	...
	public function setLnkJourEntrainementAdherents(PropelCollection $lnkJourEntrainementAdherents, PropelPDO $con = null)
	{
		...
		foreach ($lnkJourEntrainementAdherents as $lnkJourEntrainementAdherent) {
			...
			if ($lnkJourEntrainementAdherent->isNew()) {
				$lnkJourEntrainementAdherent->setRefJour($this);
			}
	 ...
	 * Otherwise if this RefJour is new, it will return
	 * an empty collection; or if this RefJour has previously
	 * been saved, it will retrieve related LnkJourEntrainementAdherents from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in RefJour.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	 ...
	 * @return     PropelCollection|array LnkJourEntrainementAdherent[] List of LnkJourEntrainementAdherent objects
	 */
	public function getLnkJourEntrainementAdherentsJoinTblAdherent($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = LnkJourEntrainementAdherentQuery::create(null, $criteria);
	...
	{
		return (string) $this->exportTo(RefJourPeer::DEFAULT_STRING_FORMAT);
	}
	...
	public function __call($name, $params)
	{
		...
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseRefJour:' . $name))
		{
