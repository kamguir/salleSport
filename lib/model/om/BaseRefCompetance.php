...
/**
 * Base class that represents a row from the 'ref_competance' table.
 *
 * 
 *
 * @package    propel.generator.lib.model.om
 */
abstract class BaseRefCompetance extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'RefCompetancePeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        RefCompetancePeer
	 */
	...
	/**
	 * The value for the competance_libelle field.
	 * @var        string
	 */
	protected $competance_libelle;
	...
	/**
	 * Get the [competance_libelle] column value.
	 * 
	 * @return     string
	 */
	public function getCompetanceLibelle()
	{
		return $this->competance_libelle;
	}
	 ...
	 * @return     RefCompetance The current object (for fluent API support)
	 */
	public function setCompetanceId($v)
	{
		...
		if ($this->competance_id !== $v) {
			...
			$this->modifiedColumns[] = RefCompetancePeer::COMPETANCE_ID;
		}
	...
	/**
	 * Set the value of [competance_libelle] column.
	 * 
	 * @param      string $v new value
	 * @return     RefCompetance The current object (for fluent API support)
	 */
	public function setCompetanceLibelle($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->competance_libelle !== $v) {
			$this->competance_libelle = $v;
			$this->modifiedColumns[] = RefCompetancePeer::COMPETANCE_LIBELLE;
		}

		return $this;
	} // setCompetanceLibelle()
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->competance_id = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			$this->competance_libelle = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->resetModified();
			...
			return $startcol + 2; // 2 = RefCompetancePeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating RefCompetance object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = RefCompetancePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = RefCompetanceQuery::create()
				->filterByPrimaryKey($this->getPrimaryKey());
			$ret = $this->preDelete($con);
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRefCompetance:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRefCompetance:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRefCompetance:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRefCompetance:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				RefCompetancePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(RefCompetancePeer::COMPETANCE_ID)) {
			$modifiedColumns[':p' . $index++]  = '`COMPETANCE_ID`';
		}
		if ($this->isColumnModified(RefCompetancePeer::COMPETANCE_LIBELLE)) {
			$modifiedColumns[':p' . $index++]  = '`COMPETANCE_LIBELLE`';
		}

		$sql = sprintf(
			'INSERT INTO `ref_competance` (%s) VALUES (%s)',
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					case '`COMPETANCE_ID`':
						...
						break;
					case '`COMPETANCE_LIBELLE`':
						$stmt->bindValue($identifier, $this->competance_libelle, PDO::PARAM_STR);
						break;
				}
			}
			$stmt->execute();
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = RefCompetancePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = RefCompetancePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['RefCompetance'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['RefCompetance'][$this->getPrimaryKey()] = true;
		$keys = RefCompetancePeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getCompetanceId(),
			$keys[1] => $this->getCompetanceLibelle(),
		);
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			case 0:
				...
				break;
			case 1:
				$this->setCompetanceLibelle($value);
				break;
		} // switch()
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setCompetanceId($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setCompetanceLibelle($arr[$keys[1]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(RefCompetancePeer::COMPETANCE_ID)) $criteria->add(RefCompetancePeer::COMPETANCE_ID, $this->competance_id);
		if ($this->isColumnModified(RefCompetancePeer::COMPETANCE_LIBELLE)) $criteria->add(RefCompetancePeer::COMPETANCE_LIBELLE, $this->competance_libelle);
	...
	{
		$criteria = new Criteria(RefCompetancePeer::DATABASE_NAME);
		$criteria->add(RefCompetancePeer::COMPETANCE_ID, $this->competance_id);
	...
	/**
	 * Generic method to set the primary key (competance_id column).
	 *
	...
	{
		return null === $this->getCompetanceId();
	}
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of RefCompetance (or compatible) type.
	...
	{
		$copyObj->setCompetanceLibelle($this->getCompetanceLibelle());
		...
		if ($makeNew) {
			$copyObj->setNew(true);
			$copyObj->setCompetanceId(NULL); // this is a auto-increment column, so set to default value
		}
	}
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     RefCompetance Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     RefCompetancePeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new RefCompetancePeer();
		}
		return self::$peer;
	}
	 ...
	 * If this RefCompetance is new, it will return
	...
	public function setTblCeintures(PropelCollection $tblCeintures, PropelPDO $con = null)
	{
		...
		foreach ($tblCeintures as $tblCeinture) {
			...
			if ($tblCeinture->isNew()) {
				$tblCeinture->setRefCompetance($this);
			}
	 ...
	 * Otherwise if this RefCompetance is new, it will return
	 * an empty collection; or if this RefCompetance has previously
	 * been saved, it will retrieve related TblCeintures from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in RefCompetance.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	...
	{
		return (string) $this->exportTo(RefCompetancePeer::DEFAULT_STRING_FORMAT);
	}
	...
	public function __call($name, $params)
	{
		...
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseRefCompetance:' . $name))
		{
