...
/**
 * Base class that represents a row from the 'ref_ville' table.
 *
 * 
 *
 * @package    propel.generator.lib.model.om
 */
abstract class BaseRefVille extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'RefVillePeer';
	...
	/**
	 * The value for the libelle_ville field.
	 * @var        string
	 */
	protected $libelle_ville;
	...
	/**
	 * Get the [libelle_ville] column value.
	 * 
	 * @return     string
	 */
	 ...
	 * @return     RefVille The current object (for fluent API support)
	 */
	public function setVilleId($v)
	{
		...
		if ($this->ville_id !== $v) {
			$this->ville_id = $v;
			$this->modifiedColumns[] = RefVillePeer::VILLE_ID;
		}
	...
	/**
	 * Set the value of [libelle_ville] column.
	 * 
	...
	public function setLibelleVille($v)
	{
		...
		if ($this->libelle_ville !== $v) {
			$this->libelle_ville = $v;
			$this->modifiedColumns[] = RefVillePeer::LIBELLE_VILLE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->ville_id = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			$this->libelle_ville = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->resetModified();
			...
			return $startcol + 2; // 2 = RefVillePeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating RefVille object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = RefVillePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = RefVilleQuery::create()
				->filterByPrimaryKey($this->getPrimaryKey());
			$ret = $this->preDelete($con);
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRefVille:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRefVille:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRefVille:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRefVille:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				RefVillePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		$this->modifiedColumns[] = RefVillePeer::VILLE_ID;
		if (null !== $this->ville_id) {
			throw new PropelException('Cannot insert a value for auto-increment primary key (' . RefVillePeer::VILLE_ID . ')');
		}

		 // check the columns in natural order for more readable SQL queries
		if ($this->isColumnModified(RefVillePeer::VILLE_ID)) {
			$modifiedColumns[':p' . $index++]  = '`VILLE_ID`';
		}
		if ($this->isColumnModified(RefVillePeer::LIBELLE_VILLE)) {
			$modifiedColumns[':p' . $index++]  = '`LIBELLE_VILLE`';
		}

		$sql = sprintf(
			'INSERT INTO `ref_ville` (%s) VALUES (%s)',
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					case '`VILLE_ID`':
						...
						break;
					case '`LIBELLE_VILLE`':
						$stmt->bindValue($identifier, $this->libelle_ville, PDO::PARAM_STR);
						break;
				}
			}
			$stmt->execute();
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = RefVillePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = RefVillePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['RefVille'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['RefVille'][$this->getPrimaryKey()] = true;
		$keys = RefVillePeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getVilleId(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setVilleId($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setLibelleVille($arr[$keys[1]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(RefVillePeer::VILLE_ID)) $criteria->add(RefVillePeer::VILLE_ID, $this->ville_id);
		if ($this->isColumnModified(RefVillePeer::LIBELLE_VILLE)) $criteria->add(RefVillePeer::LIBELLE_VILLE, $this->libelle_ville);
	...
	{
		$criteria = new Criteria(RefVillePeer::DATABASE_NAME);
		$criteria->add(RefVillePeer::VILLE_ID, $this->ville_id);
	...
	/**
	 * Generic method to set the primary key (ville_id column).
	 *
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of RefVille (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		if ($makeNew) {
			$copyObj->setNew(true);
			$copyObj->setVilleId(NULL); // this is a auto-increment column, so set to default value
		}
	}
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     RefVille Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * If this RefVille is new, it will return
	...
	public function setTblCompetitions(PropelCollection $tblCompetitions, PropelPDO $con = null)
	{
		...
		foreach ($tblCompetitions as $tblCompetition) {
			...
			if ($tblCompetition->isNew()) {
				$tblCompetition->setRefVille($this);
			}
	 ...
	 * Otherwise if this RefVille is new, it will return
	 * an empty collection; or if this RefVille has previously
	 * been saved, it will retrieve related TblCompetitions from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in RefVille.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	 ...
	 * @return     PropelCollection|array TblCompetition[] List of TblCompetition objects
	 */
	public function getTblCompetitionsJoinRefTypeSport($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = TblCompetitionQuery::create(null, $criteria);
	...
	{
		return (string) $this->exportTo(RefVillePeer::DEFAULT_STRING_FORMAT);
	}
	...
	public function __call($name, $params)
	{
		...
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseRefVille:' . $name))
		{
