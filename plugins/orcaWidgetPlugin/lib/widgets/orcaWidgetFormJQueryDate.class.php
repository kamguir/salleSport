 ...
 */
class orcaWidgetFormJQueryDate extends sfWidgetForm
{
  /**
  ...
  protected function configure($options = array(), $attributes = array())
  {
    ...
    $this->addOption('years_start');
    $this->addOption('years_stop');
    $this->addOption('format');
    $this->addOption('div', false);
    
    $format = isset($options['format'])?$options['format']:'%day%%month%%year%';
    $years_start = isset($options['years_start'])?$options['years_start']:-5;
    $years_stop = isset($options['years_stop'])?$options['years_stop']:5;
    $years_base = range(date('Y') + $years_start, date('Y') + $years_stop);
    $years = array_combine($years_base, $years_base);
    $this->addOption('date_widget', new sfWidgetFormDate(array('format'=>$format,
                                                               'years'=>$years)));
  ...
  public function render($name, $value = null, $attributes = array(), $errors = array())
  {
    ...
    return ($this->getOption('div')?"<div class='date_jquery_widget'>":null).$this->getOption('date_widget')->render($name, $value, $attributes, $errors).
           $this->renderTag('input', array('type' => 'hidden', 'size' => 10, 'id' => $id = $this->generateId($name).'_jquery_control', 'disabled' => 'disabled')).($this->getOption('div')?"</div>":null).
           sprintf(<<<EOF
